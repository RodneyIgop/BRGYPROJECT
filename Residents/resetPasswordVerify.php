...
if (empty($email) || empty($token)) {
    header('Location: forgotPassword.php?error=invalid_request');
    exit;
}
...
if ($result->num_rows === 0) {
    $stmt->close();
    header('Location: forgotPassword.php?error=invalid_token');
    exit;
}
...
if (strtotime($reset_request['expires_at']) < time()) {
    header('Location: forgotPassword.php?error=expired_token');
    exit;
}
...
<head>
  ...
  <title>Barangay New Era | Verify Reset Code</title>
...
<body>
  ...
  <script>
    ...
    // Countdown timer
    // Debug: Log the expires_at value
    console.log('Original expires_at:', '<?php echo $reset_request['expires_at']; ?>');
    console.log('Formatted expires_at:', '<?php echo date('Y-m-d\TH:i:s', strtotime($reset_request['expires_at'])); ?>');
    console.log('Timestamp:', '<?php echo strtotime($reset_request['expires_at']); ?>');
    ...
    console.log('ExpiresAt timestamp:', expiresAt);
    console.log('Current timestamp:', new Date().getTime());
    console.log('Distance:', expiresAt - new Date().getTime());
    ...
    document.getElementById('verifyCodeForm').addEventListener('submit', function(e) {
      ...
      // Send request
      fetch('resetPasswordVerifyProcess.php', {
        method: 'POST',
        body: formData
      })
    ...
    document.getElementById('resendBtn').addEventListener('click', function() {
      ...
      .then(data => {
        if (data.success) {
          ...
          window.location.href = data.redirect_url || 'resetPasswordVerify.php?email=' + encodeURIComponent('<?php echo htmlspecialchars($email); ?>');
        } else {
