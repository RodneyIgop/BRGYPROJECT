...
// Check if email exists in usertbl (resident accounts)
$stmt = $conn->prepare('SELECT UserID, FirstName, LastName FROM usertbl WHERE email = ? AND status = "active"');
...
if ($result->num_rows === 0) {
    $stmt->close();
    echo json_encode(['success' => false, 'message' => 'Email address not found or account is not active']);
    exit;
}
...
try {
    ...
    // Content
    $mail->isHTML(true);
    $mail->Subject = 'Barangay New Era - Password Reset Verification Code';
    
    $reset_link = "http://$_SERVER[HTTP_HOST]/Residents/resetPasswordVerify.php?email=" . urlencode($email) . "&token=" . urlencode($reset_token);
    
    $mail->Body = '
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="text-align: center; margin-bottom: 30px;">
                <img src="/era/images/brgylogo.png" alt="Barangay New Era Logo" style="width: 80px;">
                <h2 style="color: #014A7F; margin: 10px 0;">Barangay New Era</h2>
            </div>
            
            <h3 style="color: #333;">Password Reset Request</h3>
            
            <p>Dear ' . htmlspecialchars($user['FirstName'] . ' ' . $user['LastName']) . ',</p>
            
            <p>We received a request to reset your password for your Barangay New Era resident account. Your verification code is:</p>
    ...
    $mail->AltBody = 'Your password reset verification code is: ' . $verification_code . '. This code expires in 10 minutes. If you didn\'t request this, please ignore this email.';

    $mail->send();
    
    // Get the reset token for the redirect
    ...
    $redirect_url = 'resetPasswordVerify.php?email=' . urlencode($email) . '&token=' . urlencode($reset_request['reset_token']);
