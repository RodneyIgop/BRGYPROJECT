...
document.querySelectorAll('.reject-btn').forEach(btn=>{
  btn.addEventListener('click',e=>{e.preventDefault(); declineField.value=btn.dataset.id; declineModal.style.display='flex';});
});
// close decline modal
 if (declineClose && declineModal) declineClose.addEventListener('click',()=>declineModal.style.display='none');
 window.addEventListener('click',e=>{if(e.target===modal) modal.style.display='none'; if(e.target===declineModal) declineModal.style.display='none';});
        document.getElementById('approveBtn').addEventListener('click', () => {
            if(currentId){ window.location.href = 'adminapprove_query.php?id='+currentId; }
        });

        const searchInput = document.getElementById('pendingSearch');
        const dateInput = document.getElementById('pendingDateFilter');
        ...
        if(table && (searchInput || dateInput)){
            ...
            const getRowDateYmd = (row) => {
                const cell = row.cells && row.cells.length > 3 ? row.cells[3] : null;
                if(!cell) return '';
