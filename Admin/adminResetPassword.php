...
// Check if admin password reset session exists and is verified
if (!isset($_SESSION['admin_password_reset']) || !$_SESSION['admin_password_reset']['verified']) {
    header('Location: adminForgotPassword.php?error=session_expired');
    exit;
}

// Check if verification was done within the last 30 minutes
$verified_at = $_SESSION['admin_password_reset']['verified_at'];
...
$email = $_SESSION['admin_password_reset']['email'];
?>
...
<head>
  ...
  <title>Barangay New Era | Admin Reset Password</title>
...
<body>
  ...
  <script>
    ...
    document.getElementById('resetPasswordForm').addEventListener('submit', function(e) {
      ...
      // Send request
      fetch('adminResetPasswordProcess.php', {
        method: 'POST',
        body: formData
      })
      ...
      .then(data => {
        ...
        if (data.success) {
          ...
          window.location.href = 'adminLogin.php?success=password_reset';
        } else {
          // Show error message
          const errorDiv = document.createElement('div');
          errorDiv.className = 'error-msg';
